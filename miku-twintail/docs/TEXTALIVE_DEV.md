...
- **`findChorus()` だけでサビ演出を組んではいけない。** 課題曲では `getChoruses()` が
  1 区間しか返らないことがあり、「ハロー、フェルミ。」では 202 秒中 2〜13 秒
  （＝イントロ）だけだった。`getVocalAmplitude` の包絡を 4 秒でならし、
  **その分布の上位 30%（分位点）**を盛り上がりとして補うと、課題曲 6 曲すべてで
  2〜4 山・3〜4 割に収まる（`ontology/textalive.yaml` の `find-chorus-is-sparse`）。
  しきい値をピーク比で決めると「常時サビ」になるので注意。
- **`findChord()` の name はテンション・スラッシュ込みで返る**
  （`"EM7/F"` `"Esus2/C#"` `"G#7/C"` `"Bbm7"`）。合いの手の音程を決めるだけなら
  `/` の前を見て三和音に落とせば足りる（同 `chord-name-parsing`）。
